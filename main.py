...
def check_connection():
    ...
    try:
        requests.get("https://www.wikipedia.com/", timeout=5)
        print("OK")
        time.sleep(1)
        return True
    except:
...
def initiate_script(fake_username, fake_password):
    ...
    getUrl = f"https://begeni.vip/tools/send-follower/"
    ...
    while True:
        ...
        getUrlContentSoup = bs(siteSession.get(getUrl).text)
        r_follow_count = re.findall("<span\>(.*?)\</span>", str(getUrlContentSoup.header.div.div.style))[1]
        if not got:
    ...
    if 'ok' in isPasswordChanged.text:
        ...
        run = False
        time.sleep(1.2)
        print("\nAccount : " + fake_username + f"\033[92m  +{getInitialCount} OK\033[0m")
        instaSession.close()
